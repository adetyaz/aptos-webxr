module admin::PHYGITAL{
    ...
    const COLLECTION_NAME: vector<u8> = b"PHYGITAL";
    const COLLECTION_DESCRIPTION: vector<u8> = b"That creates digital avataar of your physical product";
    const COLLECTION_URI: vector<u8> = b"ipfs://bafybeiakibvianmzrecxzyh6oonapk7fqggburcfsseildhhgrbxh3tz2u/111nft.png";

    // Token information
    const TOKEN_DESCRIPTION: vector<u8> = b"PHYGITAL NFT";
    ...
    */
    ...
    inline fun mint_internal(user: address){
        ...
        let token_const_ref = token::create_named_token(
            ...
            string_utils::format1(&b"PHYGITAL #{}", current_nft),
            option::some(royalty),
            uri
        );
        ...
        // Create the PHYGITALToken object and move it to the new token object signer
        let new_nft_token = PHYGITALToken {
            mutator_ref: token::generate_mutator_ref(&token_const_ref),
            burn_ref: token::generate_burn_ref(&token_const_ref),
        };

        move_to<PHYGITALToken>(&obj_signer, new_nft_token);
